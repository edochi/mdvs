...
- One branch per TODO or feature: `feat/description`, `fix/description`,
  `docs/description`
...
**Never commit unless the user explicitly asks.** No autonomous commits. Wait
for "commit", "commit and push", or similar.

## Before Committing

- If the work corresponds to a spec TODO, update its status to `done` before
  committing (use the `todo` skill). This lets the user review the TODO update
...
Use [Conventional Commits](https://www.conventionalcommits.org/) format. A
...
**Only push when the user explicitly asks.** "Commit" does not imply "push".
Wait for "push", "commit and push", or similar.
