...
Record terminal sessions with asciinema and embed them in mdBook pages. Gives
readers a feel for the CLI workflow without requiring a live backend.
...
- Add the player JS/CSS via `output.html.additional-js` and
  `output.html.additional-css` in `book.toml`
...
- **Customized build**: `mdvs init` → `mdvs build --set-model ...` →
  `mdvs search`
- **Incremental build**: edit a file → `mdvs build` (shows only changed files
  re-embedded)
...
- Consider adding a `scripts/record-demos.sh` helper for reproducible recordings
  against `example_kb/`
