...
DataFusion is pulled with all default features, many of which mdvs doesn't use.
Disabling unused features should reduce compile time and binary size.
...
- `nested_expressions` — array functions (`array_has`, `array_length`) used in
  `--where`
...
- `compression` — bzip2, xz, flate2, zstd for reading compressed CSV/JSON (not
  used — Snappy for parquet is built into the parquet crate)
