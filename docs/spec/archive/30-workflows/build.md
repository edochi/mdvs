...
The build workflow processes markdown files into the `.mdvs/` artifact: extract
frontmatter, split into chunks, compute embeddings, and write compressed Parquet
files. Default mode is incremental — only changed files are reprocessed.
Analogous to `cargo build`.

The build implicitly refreshes the lock file before processing, like
...
Content hashes in `mdvs.lock` `[[file]]` entries are compared against freshly
computed hashes from the filesystem:
...
Content hash is computed over the full file content (frontmatter + body), not
just the body. This means frontmatter-only changes (e.g., updating tags) also
...
Skips the diff phase. Removes existing `.mdvs/` and processes all files from
scratch:
...
Embeddings are computed in batches for efficiency. `model2vec-rs` supports batch
inference. The batch size is an implementation detail (e.g., 256 chunks per
batch), not user-configurable.

The progress bar (via `indicatif`) shows per-file progress during the
extract+chunk phase and per-batch progress during embedding.
...
- [Terminology](../01-terminology.md) — definitions for chunk, plain text,
  content hash, build
- [Crate: mdvs](../10-crates/mdvs/spec.md) — `ingest`, `embed`, `storage`
  modules
