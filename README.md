...
**No frontmatter at all**: the note still gets indexed. The `include_bare_files` config option controls whether files without frontmatter are included in inference and validation (default: exclude them).
...
Options:
    ...
    --include-bare-files Include files without frontmatter in inference
```
...
- [x] `include_bare_files` config option for including files without frontmatter
